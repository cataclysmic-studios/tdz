...
export class TowerViewport extends BaseComponent<Attributes, ViewportFrame> implements OnStart {
  ...
  public loadModel(): void {
    ...
    const towerName = <ExtractKeys<typeof Assets.Towers, CharacterModel>>this.attributes.TowerViewport_Tower;
    const towerModel = this.towerJanitor.Add(Assets.Towers[towerName].Clone());
    towerModel.Parent = this.wm;
    towerModel.PivotTo(
      new CFrame(-0.55, 0, 0).mul(CFrame.Angles(0, math.rad(-20), 0))
    );

    this.towerJanitor.Add(towerModel.Humanoid.Animator.LoadAnimation(towerModel.Idle)).Play(0);
  }
